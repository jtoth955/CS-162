...
void Shadow::attack() {
	...
	battle4.setNumFaces(S_ATTACK_FACE);
	battle4.setNumDice(S_ATTACK_DIE);
	attack = battle4.Roll();
...
void Shadow::defense() {
	...
	battle4.setNumFaces(S_DEFEND_FACE);
	battle4.setNumDice(S_DEFEND_DIE);
	defend = battle4.Roll();	
...
*recent battle4  
Parameters - Damage received from battle4, minus CONST Armor
*********************************************************************************/
	...
	if (getStrength() > 8)
	{
		...
		{	
			...
			if((getStrength() >= 1) && (getStrength() <= 5))
			{
				...
				if(getStrength() <= 0)
				{
					cout << "\nThe Shadow has suffered a death blow and has perished in battle4. " << endl;
					setStatus(false);					
					battle4.setGoblinFactor(false);
				}
}
