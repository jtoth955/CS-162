...
void Goblin::attack() {
	...
	battle2.setNumFaces(G_ATTACK_FACE);
	battle2.setNumDice(G_ATTACK_DIE);
	attack = battle2.Roll();

	if(attack == 12)
	{
		if(battle2.getGoblinFactor() == false)
		{
			battle2.setGoblinFactor(true);
		}
...
void Goblin::defense() {
	...
	battle2.setNumFaces(G_DEFEND_FACE);
	battle2.setNumDice(G_DEFEND_DIE);
	defend = battle2.Roll();	
...
*recent battle2  
Parameters - Damage received from battle2, minus CONST Armor
*********************************************************************************/
	...
	if (getStrength() > 8)
	{
		...
		{	
			...
			if((getStrength() >= 1) && (getStrength() <= 5))
			{
				...
				if(getStrength() <= 0)
				{
					cout << "\nThe Goblin has suffered a death blow and has perished in battle2. " << endl;
					setStatus(false);					
				}
}
